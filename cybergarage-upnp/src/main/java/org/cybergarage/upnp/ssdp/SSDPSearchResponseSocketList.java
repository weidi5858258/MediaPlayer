        ...
        try {
            for (int j = 0; j < bindAddresses.length; j++) {
                ...
                if (!isValidAddress(bindAddresses[j])) {
                    AlwaysLog.e(TAG, "ready to create SSDPSearchResponseSocket bindAddresses = "
                            + bindAddresses[j] + ", it's invalid so drop it!!!");
                    continue;
                }
                ...
                AlwaysLog.i(TAG, "SSDPSearchResponseSocket create success!!!bindAddresses = " +
                        bindAddresses[j]);
